...
public class TrainerListWidget extends AbstractScrollWidget {
    ...
    private class UpdateState implements IAlgorithm {
        ...
        public UpdateState(List<Entry> entryContainer) {
            this.realtime = entryContainer == TrainerListWidget.this.entries;
        ...
        public void tick() {
            ...
            for(; this.i < trainerIds.size() && u < UPDATES_PER_TICK; this.i++, u++) {
                ...
                if(showAllTypes || this.tdm.getData(trainerId).getType() == trainerType) {
                    ...
                    if(showUndefeated || count > 0) {
                        ...
                        this.entries.add(new Entry(nameWidget, countWidget));
                        this.y += this.h;

                        if(realtime) {
                            TrainerListWidget.this.innerHeight = Math.max(getHeight() - totalInnerPadding(), this.y);
                        }
                    }
                }
            }
            ...
            if(!realtime && this.finished()) {
                TrainerListWidget.this.entries = this.entries;
                TrainerListWidget.this.innerHeight = Math.max(getHeight() - totalInnerPadding(), this.y);
            }
        }
    ...
    public TrainerListWidget(AbstractWidget parent, Font font, List<String> trainerIds) {
        ...
        this.updateState = new UpdateState(this.entries);
    }
